...
exit;
    ...
    try {
        ...
        $updateaccept = "UPDATE ".connectConstant::CONNECTION_DBNAME_MCCARTY.".accept SET `ac_st_id` = 1 where job_id =:job_id";
        $updateaccept = $conn2->prepare($updateaccept);
        $updateaccept->bindParam(':job_id', $row['job_id']);
        $updateaccept ->execute();


    }catch (PDOException  $e) {
            echo $e->getMessage();
        $seticon = 'error';
        $settext = 'ส่งผู้ป่วยไม่สำเร็จ';
    }
    while ($selectrow = $selectstmt->fetch(PDO::FETCH_ASSOC)){
        $updateaccept2 = "UPDATE ".connectConstant::CONNECTION_DBNAME_MCCARTY.".accept SET `ac_send_date_patient` = '".date('Y-m-d')."' , ac_send_time_patient ='".date('h:i:sa')."' where job_id =:job_id";
        $updateaccept2 = $conn2->prepare($updateaccept2);
        $updateaccept2->bindParam(':job_id', $row['job_id']);
        $updateaccept2 ->execute();

        $updatedetail = "UPDATE ".connectConstant::CONNECTION_DBNAME_MCCARTY.".job SET `sta_id` = 4 where job_id = :job_id";
        $updatedetailstmt = $conn2->prepare($updatedetail);
        $updatedetailstmt->bindParam(':job_id', $row['job_id']);
        $updatedetailstmt ->execute();
    }
        if($settext == 'ส่งผู้ป่วยเรียบร้อย'){
